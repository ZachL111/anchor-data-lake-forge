...
`anchor-data-lake-forge` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies lake behavior through framed sample traffic, with bounds and ordering tests and bounded memory input sets.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 2.
